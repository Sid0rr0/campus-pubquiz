...
const { mockUseGameSocket, mockUseAuth, searchParamsRef, routerRef } = vi.hoisted(() => ({
...
describe('AdminPage — session routing', () => {
  ...
  it('syncs the URL when the snapshot reports a different session (e.g. after selecting a new quiz)', async () => {
    ...
    mockUseGameSocket.mockReturnValue({
      snapshot: { progress: progress({ status: 'lobby' }), currentQuestion: null, joinCode: 'GHIJKL' },
      connectionError: null,
      sendAction: vi.fn(),
    });
    render(<AdminPage />);

    await waitFor(() => expect(routerRef.replace).toHaveBeenCalledWith('/admin?code=GHIJKL'));
  });

  it('does not sync the URL when the snapshot already matches the session code', () => {
    searchParamsRef.current = new URLSearchParams('code=ABCDEF');
    mockUseGameSocket.mockReturnValue({
      snapshot: { progress: progress({ status: 'lobby' }), currentQuestion: null, joinCode: 'ABCDEF' },
      connectionError: null,
      sendAction: vi.fn(),
    });
    render(<AdminPage />);
