...
const { mockUseGameSocket, mockFetchQuizzes, mockUseAuth, mockCloseSession, searchParamsRef, routerRef } =
  vi.hoisted(() => ({
...
describe('AdminPage — navigation', () => {
  ...
  it('does not show the Close Session button while the quiz is still running', () => {
    mockUseGameSocket.mockReturnValue({
      snapshot: { progress: progress({ status: 'question_open' }), currentQuestion: null, joinCode: 'TESTCODE' },
      connectionError: null,
      sendAction: vi.fn(),
    });
    render(<AdminPage />);
  ...
  it('closes the session and redirects to /sessions when the Close Session button is clicked', async () => {
    ...
    await waitFor(() => expect(routerRef.push).toHaveBeenCalledWith('/sessions'));
  });
  ...
  it('shows an error when closing the session fails', async () => {
    ...
    mockCloseSession.mockRejectedValue(new SessionApiError('still the default session', 409));
    render(<AdminPage />);

    await userEvent.click(screen.getByRole('button', { name: /close session/i }));

    expect(await screen.findByRole('alert')).toHaveTextContent(/still the default session/i);
