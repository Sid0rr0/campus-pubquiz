...
function renderShortcuts(overrides: Partial<UseAdminKeyboardShortcutsOptions> = {}) {
  ...
  const { rerender } = renderHook((props: UseAdminKeyboardShortcutsOptions) =>
...
describe('useAdminKeyboardShortcuts', () => {
  ...
  it('sends REVEAL_NEXT_TEAM on ArrowRight instead of ADVANCE while teams remain hidden', async () => {
    const { sendAction } = renderShortcuts({ canAdvance: true, hasUnrevealedTeams: true });
