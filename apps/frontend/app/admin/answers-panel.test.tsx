...
function liveAnswers(overrides: Partial<AnswersUpdatedPayload> = {}): AnswersUpdatedPayload {
  return {
    questionId: 101,
    question: {
      type: 'free_text',
...
describe('AnswersPanel', () => {
  it('shows the round, question position and correct answer', () => {
    render(<AnswersPanel liveAnswers={liveAnswers()} teams={TEAMS} onGrade={vi.fn()} />);

    expect(screen.getByText(/round 2 \(geography\)/i)).toHaveTextContent('Q3 of 4');
  ...
  it('grades an answered team with the correct answerId and point value', async () => {
    ...
    const onGrade = vi.fn();
    render(<AnswersPanel liveAnswers={liveAnswers()} teams={TEAMS} onGrade={onGrade} />);

    await user.click(screen.getByRole('button', { name: /grade the quizzards full points/i }));
