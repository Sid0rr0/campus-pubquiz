...
const { routerRef, mockFetchQuizDraft, mockCreateQuiz, mockUpdateQuiz, mockPreviewImport } =
  vi.hoisted(() => ({
...
vi.mock('@/app/lib/quiz-draft-api', async () => {
  const actual =
    await vi.importActual<typeof import('@/app/lib/quiz-draft-api')>('@/app/lib/quiz-draft-api');
  return {
    ...actual,
...
vi.mock('@/app/lib/import-api', async () => {
  const actual = await vi.importActual<typeof import('@/app/lib/import-api')>('@/app/lib/import-api');
  return {
    ...actual,
...
describe('QuizEditorPanel', () => {
  ...
  it('loads an existing quiz into the editor', async () => {
      ...
      rounds: [
        {
          ...
          breakAfter: true,
          questions: [
            { type: 'free_text', prompt: 'Largest planet?', answer: 'Jupiter', points: 2 },
          ],
        },
      ],
    });
    ...
    expect(screen.getByPlaceholderText(/question prompt/i)).toHaveValue('Largest planet?');
  ...
  it('reorders questions within a round via the move up/down buttons', async () => {
    ...
    mockUpdateQuiz.mockResolvedValue({ quizId: 5, roundCount: 1, questionCount: 2 });
  ...
  it('shows a load error when the quiz does not exist', async () => {
    mockFetchQuizDraft.mockRejectedValue(new QuizDraftApiError('Quiz 999 does not exist', 404));

    render(<QuizEditorPanel quizId="999" />);

    expect(await screen.findByRole('alert')).toHaveTextContent(/quiz 999 does not exist/i);
  });

  it('creates a new quiz and redirects to its stable edit url', async () => {
    const user = userEvent.setup();
    mockCreateQuiz.mockResolvedValue({ quizId: 42, roundCount: 1, questionCount: 1 });
    render(<QuizEditorPanel quizId="new" />);
    await user.click(screen.getByRole('button', { name: /start from scratch/i }));

    await user.type(screen.getByPlaceholderText(/untitled quiz/i), 'Trivia Night');
    await user.click(screen.getByRole('button', { name: /add question/i }));
    await user.type(screen.getByPlaceholderText(/question prompt/i), 'Capital of France?');
  ...
  it('updates an existing quiz in place and shows a saved flash', async () => {
    ...
    mockFetchQuizDraft.mockResolvedValue({
      ...
      title: 'Trivia Night',
      rounds: [
        { title: 'History', breakAfter: true, questions: [] },
      ],
    });
    mockUpdateQuiz.mockResolvedValue({ quizId: 5, roundCount: 1, questionCount: 0 });
    ...
    await waitFor(() => expect(mockUpdateQuiz).toHaveBeenCalledWith(5, expect.objectContaining({ title: 'Trivia Night' })));
  ...
  it('shows validation issues from a rejected save without crashing', async () => {
    ...
    expect(screen.getByText(/round 1, q1 \(prompt\): missing question text/i)).toBeInTheDocument();
  });
